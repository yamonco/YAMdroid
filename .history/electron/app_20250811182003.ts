...
ipcMain.handle("save-profiles", async (event, profiles) => {
  try {
    const baseDir = app.isPackaged
      ? join(app.getAppPath(), "../resources/profiles")
      : join(process.cwd(), "resources/profiles");
    if (!existsSync(baseDir)) mkdirSync(baseDir, { recursive: true });
    const filePath = join(baseDir, "profiles.json");
...
// 포그라운드/백그라운드 전환 (윈도우 핸들 제어는 별도 구현 필요)
ipcMain.handle("set-scrcpy-foreground", (event, { sessionId, foreground }) => {
  ...
  // 실제 창 제어는 별도 구현 필요 (윈도우 핸들 제어 등)
  session.foreground = foreground;
  // TODO: 윈도우 핸들 찾아서 show/hide 등 구현 가능
