...
function registerIpcHandlers() {
  ...
  ipcMain.handle("set-scrcpy-foreground", async (event, payload: { title: string, foreground: boolean }) => {
    ...
    if (!ok) {
      ...
      // 세션 상태 동기화: 프론트에 강제 업데이트 알림
      event.sender.send('scrcpy-session-update', { type: 'foreground', sessionId: null, foreground });
    }
...
ipcMain.handle("run-scrcpy", async (event, params: { folder: string, device?: string, resolution: string, dpi: string }) => {
  try {
    ...
    // device 값이 없으면 adb로 자동 감지
    if (!device) {
      const adbPath = join(cwd, "adb.exe");
      const adbCmd = existsSync(adbPath) ? `"${adbPath}" devices` : `adb devices`;
      const out = require('child_process').execSync(adbCmd).toString();
    ...
    // 창제목 고유화: --window-title=SCRCPY_{device} (device가 없으면 기본값 'UNKNOWN')
    const windowTitle = `SCRCPY_${device || 'UNKNOWN'}`;
