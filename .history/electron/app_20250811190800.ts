...
if ($hwnd -ne 0) {
  [void][Win]::ShowWindow($hwnd, ${foreground ? 9 : 6});
  ${foreground ? '[void][Win]::SetForegroundWindow($hwnd);' : ''}
}`;
      require('child_process').exec(`powershell -Command "${psScript.replace(/"/g, '`"')}"`, (err, stdout, stderr) => {
