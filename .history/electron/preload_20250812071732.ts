...
export const CONTEXT_BRIDGE = {
  ...
  getScrcpyFolder: async (): Promise<string> => {
    return await ipcRenderer.invoke('get-scrcpy-folder');
  },
  /** scrcpy 폴더에 aapt.exe 저장 */
  saveAaptBinary: async (folder: string, arr: Uint8Array) => {
    return await ipcRenderer.invoke('save-aapt-binary', folder, Array.from(arr));
  },
  /**
   * taskbar 폴더 내 APK 파일 목록 반환
   */
  listTaskbarApks: async (): Promise<string[]> => {
    return await ipcRenderer.invoke('list-taskbar-apks');
  },
  /**
   * 지정 APK를 adb로 설치
   */
  installApkWithAdb: async (apkPath: string) => {
    return await ipcRenderer.invoke('install-apk-with-adb', apkPath);
  },
  /**
   ...
   */
  runScrcpy: async (params: { folder: string, args: string[] }) => {
