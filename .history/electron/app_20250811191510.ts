...
function registerIpcHandlers() {
  ...
  ipcMain.handle("set-scrcpy-foreground", (event, { sessionId, foreground }) => {
    console.log(`[IPC] set-scrcpy-foreground called: sessionId=${sessionId}, foreground=${foreground}`);
    const session = scrcpySessions[sessionId];
    if (!session) {
      console.warn(`[IPC] set-scrcpy-foreground: session not found: ${sessionId}`);
      return { success: false, error: '세션 없음' };
    }
    session.foreground = foreground;
      // 윈도우 포커스/최소화 powershell 호출 (윈도우 핸들 polling)
      const pid = session.pid;
      if (pid && process.platform === 'win32') {
        const typeDef = 'using System;using System.Runtime.InteropServices;public class Win{[DllImport(\\"user32.dll\\")]public static extern bool ShowWindow(IntPtr hWnd,int nCmdShow);[DllImport(\\"user32.dll\\")]public static extern bool SetForegroundWindow(IntPtr hWnd);}';
        let psScript = `Add-Type -TypeDefinition \"${typeDef}\"; $try=0; while($try -lt 10) { $hwnd = (Get-Process -Id ${pid}).MainWindowHandle; if ($hwnd -ne 0) { [void][Win]::ShowWindow($hwnd, ${foreground ? 9 : 6});`;
        if (foreground) psScript += ' [void][Win]::SetForegroundWindow($hwnd);';
        psScript += ' break } Start-Sleep -Milliseconds 100; $try++ }';
        require('child_process').exec(`powershell -Command '${psScript}'`, (err, stdout, stderr) => {
          if (err) console.warn(`[IPC] set-scrcpy-foreground powershell error:`, err, stderr);
          else console.log(`[IPC] set-scrcpy-foreground powershell ok:`, stdout);
        });
      }
    // 모든 렌더러에 세션 업데이트 broadcast
    const { BrowserWindow } = require('electron');
    BrowserWindow.getAllWindows().forEach(win => {
      win.webContents.send("scrcpy-session-update", { type: 'foreground', sessionId, foreground });
    });
    console.log(`[IPC] set-scrcpy-foreground: session updated and broadcasted: ${sessionId}, foreground=${foreground}`);
    return { success: true };
  });
}
