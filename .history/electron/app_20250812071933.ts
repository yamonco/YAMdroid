...
ipcMain.handle('install-apk-with-adb', async (_event, apkPath: string) => {
  try {
    ...
    let pkg = '';
    // node-apk로 패키지명 추출 (공식 문법)
    try {
      const nodeApk = require('node-apk');
      const apkInfo = await nodeApk.parse(apkPath);
      // node-apk는 packageName 또는 manifest.package에 패키지명이 있음
      pkg = apkInfo?.packageName || apkInfo?.manifest?.package || '';
      if (pkg) {
        out += `\n[node-apk 패키지명 추출] ${pkg}`;
      } else {
        out += `\n[node-apk 패키지명 추출 실패: 결과 없음]`;
      }
    } catch (e) {
