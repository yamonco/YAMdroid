...
// SCRCPY 실행 IPC
ipcMain.handle("run-scrcpy", async (_, params: { folder: string, device: string, resolution: string, dpi: string }) => {
  try {
    ...
    // child_process로 scrcpy.exe 실행
    const child = spawn(exePath, args, { cwd: join(baseDir, params.folder), detached: true });
    child.unref();
    return { success: true };
  } catch (e) {
