...
ipcMain.handle('install-apk-with-adb', async (_event, apkPath: string) => {
  try {
    // 내장 adb 경로 우선 사용
    const adbDir = join(process.cwd(), 'resources', 'scrcpy', 'scrcpy-win64-v3.3.1');
    const adbPath = join(adbDir, 'adb.exe');
    const { execSync } = require('child_process');
    let adbCmd = `"${adbPath}" install -r "${apkPath}"`;
    const out = execSync(adbCmd, { encoding: 'utf8', cwd: adbDir });
