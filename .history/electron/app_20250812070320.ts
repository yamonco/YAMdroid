...
ipcMain.handle('install-apk-with-adb', async (_event, apkPath: string) => {
  try {
    // 기본적으로 adb가 PATH에 있다고 가정
    const { execSync } = require('child_process');
    const adbCmd = `adb install -r "${apkPath}"`;
    ...
    // 2. aapt가 없으면 파일명에서 추출 시도 (com.foo.bar.apk)
    if (!pkg) {
    ...
    // 3. 그래도 없으면 adb shell pm list packages -f로 역추적
    if (!pkg) {
      try {
        const listOut = execSync('adb shell pm list packages -f', { encoding: 'utf8' });
        // 파일명 일부가 포함된 패키지 찾기
        const base = apkPath.split(/[\\/]/).pop() || '';
        const found = listOut.split('\n').find(l => l.includes(base.replace(/\.apk$/i, '')));
        if (found) {
          const m = found.match(/=(.*)$/);
          if (m) pkg = m[1];
        }
    ...
    if (pkg) {
      try {
        grantLog = execSync(`adb shell pm grant ${pkg} android.permission.POST_NOTIFICATIONS`, { encoding: 'utf8' });
