...
function registerIpcHandlers() {
  ...
  ipcMain.handle("set-scrcpy-foreground", (event, { sessionId, foreground }) => {
    ...
    session.foreground = foreground;
      // 윈도우 포커스/최소화 powershell 호출 (Add-Type 한 줄 처리)
      ...
      if (pid && process.platform === 'win32') {
        ...
        let psScript = `Add-Type -TypeDefinition \"${typeDef}\"; $hwnd = (Get-Process -Id ${pid}).MainWindowHandle; if ($hwnd -ne 0) { [void][Win]::ShowWindow($hwnd, ${foreground ? 9 : 6});`;
