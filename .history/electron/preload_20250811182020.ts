...
if (!window.hasOwnProperty('bridge')) {
  contextBridge.exposeInMainWorld("bridge", {
    ...
    /** SCRCPY 세션 포그라운드/백그라운드 전환 */
    setScrcpyForeground: async (sessionId: string, foreground: boolean) => {
      return await ipcRenderer.invoke('set-scrcpy-foreground', { sessionId, foreground });
    },
