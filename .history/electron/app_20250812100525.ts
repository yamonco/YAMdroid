...
// 앱위에 그리기 권한 부여 IPC
ipcMain.handle('grant-overlay-permission', async (_event, apkPath: string) => {
  try {
    ...
    try {
      ...
      return { success: false, log: `[apkreader 패키지명 추출 실패] ${e}` };
    }
    if (!pkg) return { success: false, log: '[패키지명 추출 실패]' };
    // 권한 부여
    try {
      const out = execSync(`"${adbPath}" shell appops set ${pkg} SYSTEM_ALERT_WINDOW allow`, { encoding: 'utf8' });
      return { success: true, log: `[앱위에 그리기 권한 grant] ${pkg}: ${out}` };
    } catch (e) {
      return { success: false, log: `[앱위에 그리기 권한 grant 실패] ${pkg}: ${e}` };
    }
  } catch (e) {
...
// scrcpy 폴더 경로 반환 (가장 최신 버전 폴더)
ipcMain.handle('get-scrcpy-folder', async () => {
  try {
    const scrcpyRoot = join(process.cwd(), 'resources', 'scrcpy');
    const dirs = readdirSync(scrcpyRoot, { withFileTypes: true })
      .filter(d => d.isDirectory() && d.name.toLowerCase().includes('scrcpy'))
      .map(d => d.name)
      .sort((a, b) => b.localeCompare(a));
    if (dirs.length > 0) return join(scrcpyRoot, dirs[0]);
    return scrcpyRoot;
  } catch {
    return '';
  }
...
ipcMain.handle('install-apk-with-adb', async (_event, apkPath: string) => {
  try {
    ...
    let pkg = '';
    // node-apk로 패키지명 추출 (공식 문법: Apk 클래스로 파싱)
    try {
      ...
      await apk.close();
      pkg = manifest?.package || '';
      if (pkg) out += `\n[node-apk 패키지명 추출] ${pkg}`;
      else     out += `\n[node-apk 패키지명 추출 실패: manifest.package 없음]`;
    } catch (e) {
      out += `\n[node-apk 패키지명 추출 실패] ${e}`;
    }
    ...
    // 5. 알림권한 grant (선언 여부 검사)
    if (pkg) {
      try {
        const dumps = execSync(`"${adbPath}" shell dumpsys package ${pkg}`, { encoding: 'utf8' });
        if (!/android\.permission\.POST_NOTIFICATIONS/.test(dumps)) {
          out += `\n[경고] 앱이 POST_NOTIFICATIONS를 선언하지 않음 → grant 생략`;
        } else {
          const grantLog = execSync(`"${adbPath}" shell pm grant ${pkg} android.permission.POST_NOTIFICATIONS`, { encoding: 'utf8' });
...
app.whenReady().then(async () => {
  ...
  let mainWindow = new BrowserWindow({
    ...
    show: false,
    frame: app.isPackaged ? false : true, // 배포시 상단바 숨김
    titleBarStyle: app.isPackaged ? 'hidden' : undefined, // 배포시 상단바 숨김
    webPreferences: {
