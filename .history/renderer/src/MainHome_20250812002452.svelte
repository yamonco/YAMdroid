        ...
        <div class="flex gap-2 my-2">
          {#each profiles as p, idx}
            <button
              ...
              class:selected={idx === activeProfileIdx}
              on:click={() => handleSelectProfile(idx)}
            >{p.name || `프로필${idx + 1}`}</button>
            <button
              title="삭제"
              disabled={profiles.length <= 1}
              class="text-red-400 text-xs"
              on:click={() => handleRemoveProfile(idx)}
            >🗑️</button>
          {/each}
          <input
            class="px-1 py-0.5 border rounded text-xs w-24"
            placeholder="새 프로필명"
            bind:value={newProfileName}
            on:keydown={(e) => { if (e.key === 'Enter') handleAddProfile(); }}
          />
          <button class="text-green-400 text-xs" on:click={handleAddProfile}>＋</button>
        </div>
