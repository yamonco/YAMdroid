<script lang="ts">
  ...
  async function runScrcpy() {
    ...
    runError = '';
    try {
      // 실제 IPC 호출: window.bridge.runScrcpy 등으로 구현 필요
      // 임시: 경로/옵션 콘솔 출력
      ...
      // TODO: window.bridge.runScrcpy(params) 구현 필요
      // await window.bridge.runScrcpy(params);
      console.log('SCRCPY 실행:', params);
      alert('SCRCPY 실행: ' + JSON.stringify(params, null, 2));
    } catch (e) {
