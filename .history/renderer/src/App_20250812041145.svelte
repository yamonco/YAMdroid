  ...
  <div class="navbar bg-base-100 shadow-sm">
      <div class="navbar-start">
        <div class="dropdown" class:dropdown-open={Dropdown}>
          <button type="button" class="btn btn-ghost btn-circle" aria-label="메뉴 열기" on:click={() => dropdownOpen = !dropdownOpen}>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7" /> </svg>
          </button>
          <ul class="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow">
            <li><button type="button" class={activeNav === 'home' ? 'active' : ''} on:click={() => { activeNav = 'home'; dropdownOpen = false; }} aria-label="홈">홈</button></li>
            <li><button type="button" class={activeNav === 'setting' ? 'active' : ''} on:click={() => { activeNav = 'setting'; dropdownOpen = false; }} aria-label="설정">설정</button></li>
            <li><button type="button" class={activeNav === 'about' ? 'active' : ''} on:click={() => { activeNav = 'about'; dropdownOpen = false; }} aria-label="정보">정보</button></li>
          </ul>
        </div>
      </div>
    <div class="navbar-center">
      <span class="btn btn-ghost text-xl">YAMdroid</span>
    </div>
    <div class="navbar-end">
      <button class="btn btn-ghost btn-circle" aria-label="검색">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /> </svg>
      </button>
      <button class="btn btn-ghost btn-circle" aria-label="알림">
        <div class="indicator">
