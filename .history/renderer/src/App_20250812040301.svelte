  ...
  <div class="navbar bg-base-100 shadow-sm">
      <div class="navbar-start">
        <div class="dropdown" class:dropdown-open={dropdownOpen}>
          <div tabindex="0" role="button" class="btn btn-ghost btn-circle" on:click={() => dropdownOpen = !dropdownOpen}>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7" /> </svg>
          </div>
          <ul tabindex="0" class="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow">
            <li><a on:click={() => { activeNav = 'home'; dropdownOpen = false; }} class={activeNav === 'home' ? 'active' : ''}>홈</a></li>
            <li><a on:click={() => { activeNav = 'setting'; dropdownOpen = false; }} class={activeNav === 'setting' ? 'active' : ''}>설정</a></li>
            <li><a on:click={() => { activeNav = 'about'; dropdownOpen = false; }} class={activeNav === 'about' ? 'active' : ''}>정보</a></li>
          </ul>
        </div>
      </div>
