<script lang="ts">
  ...
  import UtilityPage from "./components/pages/UtilityPage.svelte";
  ...
  if (typeof window !== "undefined" && window.bridge?.onScrcpySessionUpdate) {
    window.bridge.onScrcpySessionUpdate((msg) => {
      if (msg.type === "start") {
        ...
        scrcpySessions = scrcpySessions.map((s) =>
          s.id === msg.sessionId ? { ...s, foreground: msg.foreground } : s,
        );
      }
    });
  ...
  function openConsole() {
    showConsole = true;
    minimizedConsole = false;
  }
  function closeConsole() {
    showConsole = false;
  }
  function toggleMinimizeConsole() {
    minimizedConsole = !minimizedConsole;
  }
...
<div class="pane-root flex flex-col w-full h-full text-zinc-200">
        ...
        >
          <li>
            <button
              ...
              onclick={() => (activeNav = "home")}>홈</button
            >
          </li>
          <li>
            <button
              type="button"
              role="menuitem"
              class={activeNav === "utility" ? "active" : ""}
              onclick={() => (activeNav = "utility")}>유틸리티</button
            >
          </li>
          <li>
            <button
              type="button"
              role="menuitem"
              class={activeNav === "setting" ? "active" : ""}
              onclick={() => (activeNav = "setting")}>설정</button
            >
          </li>
          <li>
            <button
              ...
              onclick={() => (activeNav = "about")}>정보</button
            >
          </li>
        </ul>
      </div>
  ...
  <!-- daisyUI / synthwave 테마 적용 테스트용 -->
  <div class="p-6 bg-primary text-primary-content rounded-box space-y-3" style="margin:2rem;">
    <div class="text-xl font-bold">daisyUI / synthwave 테스트</div>
    <button class="btn btn-primary">버튼 확인</button>
  </div>
</div>
