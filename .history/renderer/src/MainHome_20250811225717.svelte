      ...
      {#if selected}
        ...
        <section class="w-full bg-zinc-950 rounded-lg p-3 border border-zinc-800 flex flex-col gap-2 mb-2 mt-2">
          ...
          <input
            ...
            oninput={(e) => updateProfileField('device', (e.target as HTMLInputElement)?.value || '')}
          />
          <input
            class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs"
            placeholder="해상도 (예: 1080x2400)"
            value={profiles[activeProfileIdx].resolution}
            oninput={(e) => updateProfileField('resolution', (e.target as HTMLInputElement)?.value || '')}
          />
          <input
            class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs"
            placeholder="DPI (예: 420)"
            value={profiles[activeProfileIdx].dpi}
            oninput={(e) => updateProfileField('dpi', (e.target as HTMLInputElement)?.value || '')}
          />
        </section>
      {/if}
    {/if}
