<script lang="ts">
  ...
  async function killScrcpyAdb() {
    killRunning = true;
    try {
      const result = await window.bridge.killScrcpyAdb();
      // 로그는 scrcpy-log로 자동 출력됨
      ...
      <div class="flex flex-wrap gap-2">
        {#each profiles as profile, idx}
          <div class="flex items-center gap-1">
            <button class={`px-3 py-1 rounded-lg border ${idx === activeProfileIdx ? 'border-blue-500 bg-blue-500/20 text-blue-200' : 'border-zinc-700 bg-zinc-800 text-zinc-200 hover:bg-zinc-700'}`} on:click={() => selectProfile(idx)}>{profile.name}</button>
