  ...
  <div class="navbar bg-base-100 shadow-sm">
      <div class="navbar-start">
        <div class="dropdown" class:dropdown-open={dropdownOpen}>
          <button type="button" class="btn btn-ghost btn-circle" aria-label="메뉴 열기" onclick={() => dropdownOpen = !dropdownOpen}>
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7" /> </svg>
          </button>
          <ul class="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow">
            <li><button type="button" class={activeNav === 'home' ? 'active' : ''} onclick={() => { activeNav = 'home'; dropdownOpen = false; }} aria-label="홈">홈</button></li>
            <li><button type="button" class={activeNav === 'setting' ? 'active' : ''} onclick={() => { activeNav = 'setting'; dropdownOpen = false; }} aria-label="설정">설정</button></li>
            <li><button type="button" class={activeNav === 'about' ? 'active' : ''} onclick={() => { activeNav = 'about'; dropdownOpen = false; }} aria-label="정보">정보</button></li>
          </ul>
        </div>
      </div>
