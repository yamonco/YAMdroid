...
<div class="flex gap-4 my-4 flex-wrap">
  ...
  <div class="card bg-base-100 w-96 shadow-sm">
    <div class="card-body">
      ...
      <input type="file" class="file-input file-input-bordered w-full" accept=".json" on:change={e => exportFile = e.target.files[0]} />
      <div class="text-xs mt-1 opacity-60">기본 파일명: <span class="font-mono">{defaultExportName}</span></div>
      <div class="card-actions justify-end mt-2">
        <button class="btn btn-primary btn-sm" on:click={handleExport} disabled={!exportFile}>내보내기</button>
      </div>
  ...
  <div class="card bg-base-100 w-96 shadow-sm">
    <div class="card-body">
      ...
      <input type="file" class="file-input file-input-bordered w-full" accept=".json" on:change={e => importFile = e.target.files[0]} />
      <div class="card-actions justify-end mt-2">
        <button class="btn btn-secondary btn-sm" on:click={handleImport} disabled={!importFile}>불러오기</button>
      </div>
