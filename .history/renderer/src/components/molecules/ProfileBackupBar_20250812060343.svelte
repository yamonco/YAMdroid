...
async function handleExport() {
  try {
    // Electron의 showSaveDialog로 기본 탐색기(저장) 다이얼로그 호출
    const result = await window.bridge?.showSaveDialog?.({
      title: '프로필 백업 파일 저장',
      defaultPath: defaultExportName,
      filters: [{ name: 'JSON', extensions: ['json'] }]
    });
    // Electron의 showSaveDialog는 { filePath, canceled } 형태 반환이 일반적
  ...
  <div class="card bg-base-100 w-96 shadow-sm">
    <div class="card-body">
      ...
      <div class="card-actions justify-end mt-2">
        <button class="btn btn-primary btn-sm" on:click={handleExport}>내보내기(저장)</button>
      </div>
