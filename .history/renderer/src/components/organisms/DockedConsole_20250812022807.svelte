...
<div class="w-full flex flex-col-reverse items-center justify-end pt-2 pb-0 bg-transparent">
  {#if show}
    <div class={`w-full max-w-2xl mx-auto bg-zinc-950 border border-zinc-800 rounded-lg shadow-lg p-2 mb-2 ${minimized ? 'h-12 overflow-hidden' : 'h-64 overflow-y-auto'}`}>
  ...
  {/if}
  {#if !show}
    <div class="flex justify-end p-2 w-full max-w-2xl mx-auto">
      <Button
        variant="ghost"
        color="success"
