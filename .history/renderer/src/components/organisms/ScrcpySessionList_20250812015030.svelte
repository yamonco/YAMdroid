          ...
          <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2 ml-2">
              <button class="px-3 py-1 rounded bg-blue-700 text-xs text-white border border-blue-500 hover:bg-blue-800 transition" onclick={() => onForeground(session)} disabled={session.foreground}>
                ...
                <button
                  class="px-3 py-1 rounded text-xs text-white border transition"
                  class:bg-blue-700={!session.foreground}
                  class:bg-gray-700={session.foreground}
                  class:border-blue-500={!session.foreground}
                  class:border-gray-500={session.foreground}
                  class:hover:bg-blue-800={!session.foreground}
                  class:hover:bg-gray-800={session.foreground}
