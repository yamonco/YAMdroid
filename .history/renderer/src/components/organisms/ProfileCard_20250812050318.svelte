...
import ProfileCardMolecule from '../molecules/ProfileCardMolecule.svelte';
...
<div class="flex gap-2 flex-wrap mb-2">
        {#each profiles as p, idx}
            <ProfileCardMolecule
                name={p.name || `프로필${idx + 1}`}
                active={idx === activeProfileIdx}
                onSelect={() => onSelectProfile(idx)}
                onRemove={profiles.length > 1 ? () => onRemoveProfile(idx) : null}
            />
        {/each}
    <input
