...
export let onNewProfileNameInput: (v: string) => void = (v: string) => {};
</script>
...
<section class="w-full bg-zinc-950 rounded-lg p-3 border border-zinc-800 flex flex-col gap-2 mb-2">
  ...
  <div class="flex gap-2 mb-2">
    <input
      ...
      on:input={(e: Event) => onNewProfileNameInput((e.target as HTMLInputElement)?.value ?? '')}
      bind:value={newProfileName}
      on:input={e => onNewProfileNameInput((e.target as HTMLInputElement)?.value ?? '')}
    />
    <button
