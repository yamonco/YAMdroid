...
<div class="p-6 text-zinc-200">
  ...
  <div class="mb-6">
    ...
    <Dropdown
      options={scrcpyFolders.map(f => ({ label: f, value: f }))}
      bind:value={selectedFolder}
      placeholder="폴더 선택..."
      className="mb-2"
    />
    <button
      class="px-4 py-2 rounded bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold"
      onclick={handleSave}
      disabled={!selectedFolder}
    >
      경로 저장
    </button>
    <div class="text-xs text-zinc-400 mt-2">scrcpy.exe가 포함된 폴더를 선택하세요.</div>
  </div>
