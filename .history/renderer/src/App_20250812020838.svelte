// MainHome.svelte 전체 코드 복사 (아래에 붙여넣기)
<script lang="ts">
import './app.pane.css';
      ...
      ...
            ...
            {:else}
              ...
              <script>
                ...
                // scrcpyFolders, selected는 기존 상태 그대로 사용
                const folderOptions = scrcpyFolders.map(f => ({ label: f, value: f }));
              </script>
              <Dropdown
            ...
            <ScrcpySessionList
              ...
              onToggleForeground={(session) => setSessionForeground(session, !session.foreground)}
              onKill={killSession}
            />
