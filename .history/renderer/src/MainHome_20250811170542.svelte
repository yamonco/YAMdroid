<script lang="ts">
  ...
  function toggleMinimizeConsole() { minimizedConsole = !minimizedConsole; }
    ...
    <section class="w-full flex flex-col gap-2 mb-4">
      <label for="scrcpy-folder-select" class="block text-sm font-semibold mb-1">SCRCPY 버전 폴더</label>
      {#if loading}
        <div class="text-xs text-zinc-400">폴더 검색 중...</div>
      {:else if scrcpyFolders.length === 0}
        <div class="text-xs text-red-400">
          SCRCPY 폴더가 없습니다.<br>
          <span class="text-zinc-300">아래와 같이 폴더 구조를 만들어주세요:</span>
          <pre class="bg-zinc-950 border border-zinc-700 rounded p-2 mt-2 text-xs text-zinc-200 select-text">
    <section class="w-full flex flex-col gap-2 mb-4">
      <label for="scrcpy-folder-select" class="block text-sm font-semibold mb-1">SCRCPY 버전 폴더</label>
      {#if loading}
        <div class="text-xs text-zinc-400">폴더 검색 중...</div>
      ...
      {:else}
        <select id="scrcpy-folder-select" class="w-full px-3 py-2 rounded bg-zinc-950 border border-zinc-700 focus:ring-2 focus:ring-blue-500 outline-none text-sm" bind:value={selected}>
        ...
        {#if selected}
          <div class="mt-1 text-green-400 text-xs">선택: <span class="font-mono">{selected}</span></div>
        {/if}
      {/if}
    </section>
          <span class="text-zinc-300">아래와 같이 폴더 구조를 만들어주세요:</span>
      ...
      scrcpyLogs = [...scrcpyLogs, '[SCRCPY/ADB 종료 오류]'];
    } finally {
      killRunning = false;
    }
  }
...
resources/
  scrcpy/
    ...
    <section class="w-full bg-zinc-950 rounded-lg p-3 border border-zinc-800 flex flex-col gap-2 mb-2">
      ...
      <input class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs" placeholder="기기명 (예: Galaxy S23)" bind:value={profiles[activeProfileIdx].device} on:input={(e) => updateProfileField('device', e.target.value)} />
      <input class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs" placeholder="해상도 (예: 1080x2400)" bind:value={profiles[activeProfileIdx].resolution} on:input={(e) => updateProfileField('resolution', e.target.value)} />
      <input class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs" placeholder="DPI (예: 420)" bind:value={profiles[activeProfileIdx].dpi} on:input={(e) => updateProfileField('dpi', e.target.value)} />
    </section>
    ...
    <button class="w-full mt-2 px-6 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white font-semibold transition disabled:opacity-50" on:click={killScrcpyAdb} disabled={killRunning}>
