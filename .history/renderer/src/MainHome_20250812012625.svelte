...
<div class="pane-root flex flex-col w-full h-full text-zinc-200">
  <div class="pane-content flex-1 flex flex-col">
    <div class="pane-main flex-1 flex flex-col">
      <div class="pane-row flex flex-row flex-1 min-h-0">
        <div class="pane-section flex flex-col min-w-0 min-h-0">
          <!-- 좌측: 추후 확장용 (예: 네비, 정보 등) -->
        </div>
        <div class="pane-section flex flex-col items-center justify-center min-w-0 min-h-0">
          <div class="w-full max-w-lg bg-zinc-900 rounded-xl shadow-lg p-8 border border-zinc-800 flex flex-col items-center flex-shrink-0">
            {#if loading}
      ...
      ...
            ...
            {:else}
              ...
              {#if selected}
                ...
                <!-- 최신 아토믹 패턴 ProfilePage만 사용 (중복 제거) -->
                <ProfilePage
                  profiles={profiles}
