    ...
    <div class="navbar-start">
      <div class="dropdown">
        ...
        <ul tabindex="0" class="menu menu-sm dropdown-content bg-base-100 rounded-box z-1 mt-3 w-52 p-2 shadow">
          <li><a on:click={() => activeNav = 'home'} class={activeNav === 'home' ? 'active' : ''}>홈</a></li>
          <li><a on:click={() => activeNav = 'setting'} class={activeNav === 'setting' ? 'active' : ''}>설정</a></li>
          <li><a on:click={() => activeNav = 'about'} class={activeNav === 'about' ? 'active' : ''}>정보</a></li>
        </ul>
      </div>
    </div>
    ...
    <div class="flex-1 min-h-0 overflow-y-auto">
      {#if activeNav === 'home'}
        <ProfilePage
          ...
          toast={null}
          activeTab={activeTab}
          onTabChange={tab => activeTab = tab as 'video' | 'device'}
