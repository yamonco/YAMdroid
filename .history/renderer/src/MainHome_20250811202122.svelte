<script lang="ts">
  ...
  const scrcpySessions = rune<ScrcpySession[]>([]);

  // 세션 목록 불러오기 및 실시간 감시
  async function refreshScrcpySessions() {
    scrcpySessions.set(await window.bridge.listScrcpySessions());
    console.log('[refreshScrcpySessions] scrcpySessions:', scrcpySessions.get());
  }
  if (typeof window !== 'undefined' && window.bridge?.onScrcpySessionUpdate) {
    window.bridge.onScrcpySessionUpdate((msg) => {
      if (msg.type === 'start') {
        scrcpySessions.set([...scrcpySessions.get(), msg.session]);
      } else if (msg.type === 'end') {
        scrcpySessions.set(scrcpySessions.get().filter(s => s.id !== msg.sessionId));
      } else if (msg.type === 'foreground') {
        scrcpySessions.set(scrcpySessions.get().map(s => s.id === msg.sessionId ? { ...s, foreground: msg.foreground } : s));
      }
    });
    // 최초 로딩
  ...
  async function toggleSessionForeground(session: ScrcpySession) {
    ...
    console.log('[toggleSessionForeground] title:', title, 'foreground:', !session.foreground);
    const result = await window.bridge.setScrcpyForeground(title, !session.foreground);
    console.log('[toggleSessionForeground] result:', result);
    // Rune 방식: foreground 상태를 직접 갱신하지 않고, 백엔드에서 push된 이벤트로만 갱신
  }
          ...
          {#each $scrcpySessions as session}
            <div class="bg-zinc-800 rounded-lg p-4 flex flex-col gap-2 border border-zinc-700 shadow">
              <div class="flex items-center justify-between">
                ...
                <button class="px-3 py-1 rounded bg-gray-700 text-xs text-white ml-2 border border-gray-500 hover:bg-gray-600 transition" on:click={() => toggleSessionForeground(session)}>
