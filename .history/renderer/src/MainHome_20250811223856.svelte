  ...
  scrcpy/
    ...
    {:else}
      ...
      {#if selected}
        ...
        <!-- 선택된 프로필 상세 설정 -->
        <section class="w-full bg-zinc-950 rounded-lg p-3 border border-zinc-800 flex flex-col gap-2 mb-2 mt-2">
          <div class="font-semibold text-blue-300 text-xs mb-1">프로필 설정</div>
          <input
            class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs"
            placeholder="기기명 (예: Galaxy S23)"
            bind:value={profiles[activeProfileIdx].device}
            on:input={(e) => updateProfileField('device', (e.target as HTMLInputElement)?.value || '')}
          />
          <input
            class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs"
            placeholder="해상도 (예: 1080x2400)"
            bind:value={profiles[activeProfileIdx].resolution}
            on:input={(e) => updateProfileField('resolution', (e.target as HTMLInputElement)?.value || '')}
          />
          <input
            class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs"
            placeholder="DPI (예: 420)"
            bind:value={profiles[activeProfileIdx].dpi}
            on:input={(e) => updateProfileField('dpi', (e.target as HTMLInputElement)?.value || '')}
          />
        </section>
      {/if}
    {/if}
