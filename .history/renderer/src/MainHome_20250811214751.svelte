    ...
    <div class="w-full mb-8">
      <div class="bg-zinc-900 rounded-xl shadow p-5 border border-zinc-800 mb-4">
        ...
        <section class="w-full flex flex-col gap-2 mb-4">
          <div class="flex items-center gap-2 mb-1">
            <span class="font-semibold text-sm">프로필 목록</span>
              <input class="px-2 py-1 rounded bg-zinc-800 border border-zinc-700 text-xs w-28" placeholder="새 프로필명" value={newProfileName()} on:input={e => newProfileName.set(e.target.value)} on:keydown={(e) => e.key === 'Enter' && addProfile()} />
        ...
        <section class="w-full bg-zinc-950 rounded-lg p-3 border border-zinc-800 flex flex-col gap-2 mb-2">
          ...
          <input class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs" placeholder="기기명 (예: Galaxy S23)" value={profiles()[activeProfileIdx()].device} on:input={e => updateProfileField('device', e.target.value)} />
          <input class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs" placeholder="해상도 (예: 1080x2400)" value={profiles()[activeProfileIdx()].resolution} on:input={e => updateProfileField('resolution', e.target.value)} />
          <input class="px-3 py-2 rounded bg-zinc-900 border border-zinc-700 text-xs" placeholder="DPI (예: 420)" value={profiles()[activeProfileIdx()].dpi} on:input={e => updateProfileField('dpi', e.target.value)} />
        </section>
