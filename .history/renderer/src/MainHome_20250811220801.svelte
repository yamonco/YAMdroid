...
declare global {
  ...
  async function setSessionForeground(session: ScrcpySession, foreground: boolean) {
    ...
    if ((window.bridge as Bridge).setScrcpyForeground) {
      await (window.bridge as Bridge).setScrcpyForeground!(title, foreground);
    } else {
  ...
  async function killSession(session: ScrcpySession) {
    ...
    if ((window.bridge as Bridge).killScrcpySession) {
      await (window.bridge as Bridge).killScrcpySession!(session.pid);
    } else {
  ...
  async function refreshScrcpySessions() {
    if ((window.bridge as Bridge).listScrcpySessions) {
      scrcpySessions = await (window.bridge as Bridge).listScrcpySessions!();
      console.log('[refreshScrcpySessions] scrcpySessions:', scrcpySessions);
    } else {
      if (isDev) console.warn('listScrcpySessions 미구현');
    }
  }
  if (typeof window !== 'undefined' && (window.bridge as Bridge)?.onScrcpySessionUpdate) {
    (window.bridge as Bridge).onScrcpySessionUpdate!((msg: any) => {
      if (msg.type === 'start') {
...
// ...기존 script 코드만 남기고, 템플릿/HTML/Svelte 문법은 모두 제거...
  ...
  async function saveSelection() {
    showSaved = false;
    try {
      if ((window.bridge as Bridge).saveProfiles) {
        const result = await (window.bridge as Bridge).saveProfiles!(profiles);
        if (result.success) {
          showSaved = true;
        } else {
