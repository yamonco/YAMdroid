...
export function hideScrcpyWindowByTitle(title: string) {
  ...
  if (!h) return false;
  const result = ShowWindow(h, SW_HIDE);
  if (!result) {
    const err = GetLastError();
    console.warn('[winapi] ShowWindow(SW_HIDE) failed, HWND:', h, 'GetLastError:', err);
  }
  return result;
}
...
export function showScrcpyWindowByTitle(title: string) {
  const h = hwndByTitle(title);
  if (!h) return false;
  const result = ShowWindow(h, SW_SHOW);
  if (!result) {
    const err = GetLastError();
    console.warn('[winapi] ShowWindow(SW_SHOW) failed, HWND:', h, 'GetLastError:', err);
  }
  return result;
}
...
export function focusScrcpyWindowByTitle(title:string){
  ...
  let result = ShowWindow(h, SW_SHOW);
  if (!result) {
    const err = GetLastError();
    console.warn('[winapi] ShowWindow(SW_SHOW) failed (focus), HWND:', h, 'GetLastError:', err);
  }
  AllowSetForegroundWindow(-1);
  result = ShowWindow(h, SW_RESTORE);
  if (!result) {
    const err = GetLastError();
    console.warn('[winapi] ShowWindow(SW_RESTORE) failed, HWND:', h, 'GetLastError:', err);
  }
