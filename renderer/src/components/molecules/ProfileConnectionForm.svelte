<script lang="ts">
  ...
  export let value = {
    ...
    vdResolution: '', // 예: 1920x1080
    vdDpi: '', // 예: 420
    vdImePolicy: '', // local, fallback, hide
    vdNoSystemDecor: false,
    vdNoDestroyContent: false,
    // 오디오
    audioSource: '',
    audioCodec: '',
    audioBitrate: '',
...
<div class="space-y-6">
  ...
  <div class="mt-8 space-y-6 border-t pt-6">
    <!-- 가상데스크탑(가상 디스플레이) 옵션 -->
    <div>
      <div class="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-1">가상데스크탑(가상 디스플레이)</div>
      <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 mb-2">
        <input type="checkbox" class="h-4 w-4 accent-blue-600" checked={value.virtualDisplay} on:change={e => onChange('virtualDisplay', (e.target as HTMLInputElement).checked)} />
        가상데스크탑 모드 활성화 (--new-display)
      </label>
      {#if value.virtualDisplay}
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-2">
          <label class="flex flex-col gap-1">
            <span class="text-xs font-medium text-gray-600 dark:text-gray-400">해상도(WxH)</span>
            <input value={value.vdResolution} on:input={e => onChange('vdResolution', (e.target as HTMLInputElement)?.value)} placeholder="1920x1080" class="h-9 w-full rounded-md border border-gray-300 dark:border-gray-700 px-3 text-sm" />
          </label>
          <label class="flex flex-col gap-1">
            <span class="text-xs font-medium text-gray-600 dark:text-gray-400">DPI</span>
            <input type="number" min="0" value={value.vdDpi} on:input={e => onChange('vdDpi', (e.target as HTMLInputElement)?.value)} placeholder="420" class="h-9 w-full rounded-md border border-gray-300 dark:border-gray-700 px-3 text-sm" />
          </label>
          <label class="flex flex-col gap-1">
            <span class="text-xs font-medium text-gray-600 dark:text-gray-400">IME 정책</span>
            <select value={value.vdImePolicy} on:change={e => onChange('vdImePolicy', (e.target as HTMLSelectElement)?.value)} class="h-9 w-full rounded-md border border-gray-300 dark:border-gray-700 px-3 text-sm">
        ...
        <div class="flex flex-wrap gap-6 mb-2">
          <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            <input type="checkbox" class="h-4 w-4 accent-blue-600" checked={value.vdNoSystemDecor} on:change={e => onChange('vdNoSystemDecor', (e.target as HTMLInputElement).checked)} />
            시스템 데코 제거 (--no-vd-system-decorations)
          </label>
          <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            <input type="checkbox" class="h-4 w-4 accent-blue-600" checked={value.vdNoDestroyContent} on:change={e => onChange('vdNoDestroyContent', (e.target as HTMLInputElement).checked)} />
            종료 시 콘텐츠 파괴 금지 (--no-vd-destroy-content)
          </label>
        </div>
      {/if}
    </div>
    <div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <label class="flex flex-col gap-1">
          ...
          <input type="number" min="0" value={value.maxSize} on:input={e => onChange('maxSize', (e.target as HTMLInputElement)?.value)} placeholder="1080" class="h-9 w-full rounded-md border border-gray-300 dark:border-gray-700 px-3 text-sm" disabled={value.virtualDisplay} />
        </label>
        <label class="flex flex-col gap-1">
          <span class="text-xs font-medium text-gray-600 dark:text-gray-400">FPS 상한</span>
          <input type="number" min="0" value={value.maxFps} on:input={e => onChange('maxFps', (e.target as HTMLInputElement)?.value)} placeholder="60" class="h-9 w-full rounded-md border border-gray-300 dark:border-gray-700 px-3 text-sm" disabled={value.virtualDisplay} />
        </label>
        <label class="flex flex-col gap-1">
          <span class="text-xs font-medium text-gray-600 dark:text-gray-400">비디오 코덱</span>
          <select value={value.videoCodec} on:change={e => onChange('videoCodec', (e.target as HTMLSelectElement)?.value)} class="h-9 w-full rounded-md border border-gray-300 dark:border-gray-700 px-3 text-sm" disabled={value.virtualDisplay}>
            <option value="">기본</option>
        ...
        <label class="flex flex-col gap-1">
          ...
          <input value={value.videoBitrate} on:input={e => onChange('videoBitrate', (e.target as HTMLInputElement)?.value)} placeholder="8M" class="h-9 w-full rounded-md border border-gray-300 dark:border-gray-700 px-3 text-sm" disabled={value.virtualDisplay} />
        </label>
      </div>
    </div>
    <div>
